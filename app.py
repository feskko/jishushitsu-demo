...
menu = st.radio("メニュー", ["一括入力", "個別入力", "ランキング", "分析ダッシュボード", "データ管理"], horizontal=True, label_visibility="collapsed")
...
if missing_dates:
    ...
    missing_warning_html = f"<div style='background-color: rgba(220, 38, 38, 0.1); border-left: 4px solid #DC2626; padding: 12px 15px; border-radius: 4px; margin-bottom: 20px;'><p style='color:#DC2626; font-weight:bold; margin:0;'>⚠️ 今月の未入力日が存在します: {missing_str}</p></div>"
...
if menu == "一括入力":
    ...
    f_date_batch = st.date_input("対象日", jst_now.date(), max_value=jst_now.date())
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "開始時間": st.column_config.TextColumn("開始時刻", width="small"),
            "終了時間": st.column_config.TextColumn("終了時刻", width="small"),
        },
    ...
    if st.button("全データを保存", type="primary"):
        valid_rows = edited_df[edited_df["氏名"].str.strip() != ""]
        if valid_rows.empty: st.error("入力されている行がありません。")
        else:
            ...
            for idx, row in valid_rows.iterrows():
                ...
                if is_dup_current or is_dup_new:
                    error_msgs.append(f"{name} (重複データ)"); continue
            ...
            if new_records:
                ...
                st.session_state.sys_msg = f"{len(new_records)}件のデータを保存しました。"
                st.rerun()
    ...
    selected_user = st.selectbox("過去データからの自動補完", user_list)
    if selected_user != "-- 新規入力 --":
    ...
    with col_in: in_time_str = st.text_input("開始時刻", key=in_key, on_change=format_time_input, args=(in_key,))
    with col_out: out_time_str = st.text_input("終了時刻", key=out_key, on_change=format_time_input, args=(out_key,))
    ...
    if st.button("記録を保存", type="primary"):
        ...
        if not f_name_clean: st.error("氏名を入力してください。")
        ...
        elif in_time is None or out_time is None: st.error("時刻を正しく入力してください。")
            ...
            if duration <= 0: st.error("終了時刻は開始時刻以降に設定してください")
            else:
                df = load_data()
                ...
                if is_dup: st.error("重複データが存在します。")
                else:
                    ...
                    save_data(df)
                    st.session_state.form_key += 1 
                    st.session_state.sys_msg = f"{f_name_clean}さんの記録を保存しました。"
                    st.rerun()
    ...
    def render_premium_cards(agg):
        ...
        for i, row in top_rows.iterrows():
            ...
            grade_disp = row['学年'] if pd.notnull(row['学年']) and row['学年'] != "" else "学年未定"
    ...
    def render_section_ranking(full_agg, target_grades, section_title):
        ...
        if section_df.empty: 
            st.markdown("<p style='opacity: 0.5;'>データなし</p>", unsafe_allow_html=True)
            return
        ...
        st.dataframe(section_df[['順位', '名前', '学年', '利用時間（時間）']], use_container_width=True, hide_index=True, column_config={
            ...
            "利用時間（時間）": st.column_config.ProgressColumn("累計時間", format="%.1f", min_value=0, max_value=float(section_df['利用時間（時間）'].max() if section_df['利用時間（時間）'].max() > 0 else 1))
        })
    ...
    if not df.empty:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["今月", f"{last_month_num}月", "直近3ヶ月", "累計"])
        ...
        for tab, agg_data, period_name in zip([tab1, tab2, tab3, tab4], [get_agg(df_this_month), get_agg(df_last_month), get_agg(df_3months), get_agg(df_vp)], ["今月", f"{last_month_num}月", "直近3ヶ月", "累計"]):
            with tab:
                if agg_data.empty: st.markdown("<p style='opacity: 0.5;'>データがありません。</p>", unsafe_allow_html=True)
                else:
                    render_section_ranking(agg_data, [f"小{i}" for i in range(1, 7)], "Elementary")
                    render_section_ranking(agg_data, [f"中{i}" for i in range(1, 4)], "Junior High")
                    render_section_ranking(agg_data, ["高1", "高2"], "High School (1-2)")
                    render_section_ranking(agg_data, ["高3", "既卒/その他", ""], "High School (3 & Others)")
    ...
    if not df_ana.empty:
        ...
        st.markdown(f"<div class='section-title'>Overview (MoM)</div>", unsafe_allow_html=True)
        ...
        col_met_l1.metric(f"Total Hours ({last_month_start.month}月)", f"{hours_last:.1f} h", f"{pct_hours_last:+.1f}%")
        col_met_l2.metric(f"Active Users ({last_month_start.month}月)", f"{users_last}", f"{pct_users_last:+.1f}%")
        if users_last > 0:
            ...
            col_met_l3.metric("Avg. Hours/User", f"{avg_last:.1f} h", f"{pct_avg_last:+.1f}%")

        st.markdown("<div class='section-title'>Current Month Progress</div>", unsafe_allow_html=True)
        ...
        col_met_p1.metric(f"Total Hours (to date)", f"{hours_this_p:.1f} h", f"{pct_hours_p:+.1f}% vs last month")
        col_met_p2.metric(f"Active Users (to date)", f"{users_this_p}", f"{pct_users_p:+.1f}% vs last month")
        if users_this_p > 0:
            ...
            col_met_p3.metric("Avg. Hours/User", f"{avg_this_p:.1f} h", f"{pct_avg_p:+.1f}% vs last month")
        ...
        st.markdown(f"""
        <div style='background-color: var(--secondary-background-color); border-left: 4px solid var(--primary-color); padding: 15px 20px; border-radius: 8px; margin-top: 10px;'>
            <div style='font-size: 0.9rem; color: var(--text-color); opacity: 0.8;'>Projected Next Month Traffic</div>
            <div style='color: var(--text-color); font-size: 1.5rem; font-weight: 800; margin-top: 4px;'>
                {next_month_h:.0f} <span style='font-size: 1rem; font-weight: normal; opacity: 0.7;'>HOURS</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        
    st.markdown("<hr style='margin: 40px 0; border-color: rgba(128,128,128,0.2);'>", unsafe_allow_html=True)

    tab1, tab2, tab3 = st.tabs(["Heatmap", "User Analytics", "Traffic Prediction"])
    ...
    with tab1:
        ...
        if not df_ana.empty:
            ...
            selected_period = st.selectbox("Period", month_options, label_visibility="collapsed")
            ...
            for tb in current_time_slots: html += f"<th>{tb[:2]}:00</th>"
            html += "</tr>"
            for wd in weekdays:
    ...
    with tab2:
        ...
        if not df_ana.empty:
            ...
            if unique_names:
                selected_name = st.selectbox("Search User", ["-- Select User --"] + unique_names, label_visibility="collapsed")
                if selected_name != "-- Select User --":
                    ...
                    st.markdown(f"""
                    <div style='background-color: var(--secondary-background-color); padding: 24px; border-radius: 12px; margin-bottom: 24px;'>
                        <div style='font-size: 1rem; color: var(--text-color); opacity: 0.7; margin-bottom: 8px;'>{selected_name} - Current Month</div>
                        <div style='font-size: 3rem; font-weight: 900; color: var(--primary-color); line-height: 1;'>{total_h:.1f} <span style='font-size: 1.2rem; font-weight: 700; color: var(--text-color); opacity: 0.6;'>HOURS</span></div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    st.markdown("<p style='font-weight: 600; margin-top: 20px;'>Recent Logs</p>", unsafe_allow_html=True)
    ...
    with tab3:
        st.markdown("<div class='section-title'>Forecast Model (Next 7 Days)</div>", unsafe_allow_html=True)
        if not df_ana.empty:
            ...
            if has_test or has_before:
                st.markdown(f"<div style='background-color: rgba(234, 88, 12, 0.1); border-left: 4px solid #EA580C; padding: 12px 15px; margin-bottom: 20px; border-radius: 4px;'><p style='color:#EA580C; font-weight:bold; margin:0; font-size: 0.9rem;'>⚠ Event Detected: Adjustment factors (x{test_mult:.1f} - x{before_mult:.1f}) applied to prediction model.</p></div>", unsafe_allow_html=True)
            ...
            if not df_recent.empty:
                ...
                for wd in weekdays:
                    ...
                    for tb in pred_time_slots:
                        ...
                        if rounded_val >= 20: bg_color, font_color, display_val = "rgba(220, 38, 38, 0.9)", "#FFFFFF", "MAX"
                        elif rounded_val >= 15: bg_color, font_color, display_val = f"rgba(234, 88, 12, {max(0.6, ratio)})", "#FFFFFF", f"{rounded_val}"
                        elif rounded_val > 0: bg_color, font_color, display_val = f"rgba(37, 99, 235, {ratio * 0.8})", ("#FFFFFF" if ratio > 0.4 else "var(--text-color)"), f"{rounded_val}"
                ...
                html += "</table></div>"
                st.markdown(html, unsafe_allow_html=True)
            else: st.info("Insufficient data for prediction model.")

elif menu == "データ管理":
    st.markdown("<div class='main-title'>Database Management</div>", unsafe_allow_html=True)
    df_manage = load_data()
    
    if not df_manage.empty:
        tab_edit, tab_del = st.tabs(["Edit Record", "Batch Delete"])
        options = []
        ...
        with tab_del:
            selected_dels = st.multiselect("Select records to delete", options, format_func=lambda x: x[1])
            if st.button("Delete Selected", type="primary"):
                if selected_dels:
                    indices_to_drop = [int(x[0]) for x in selected_dels]
                    df_manage = df_manage.drop(indices_to_drop).reset_index(drop=True)
                    save_data(df_manage)
                    st.session_state.sys_msg = f"Deleted {len(indices_to_drop)} records."
                    st.rerun()
        ...
        with tab_edit:
            ...
            if selected_mng[0] != "-1":
                ...
                st.markdown("<div style='margin-top: 10px; padding: 24px; border-radius: 8px; background-color: var(--secondary-background-color);'>", unsafe_allow_html=True)
                ...
                with col_btn1:
                    if st.button("Save Changes", use_container_width=True, type="primary"):
                        ...
                        if edit_name_clean and edit_grade != "--選択--" and edit_in and edit_out:
                            ...
                            if duration > 0:
                                ...
                                save_data(df_manage)
                                st.session_state.sys_msg = "Record updated."
                                st.rerun()
                            else: st.error("Invalid time span.")
